...
# 具体数字（人类特征）
SPECIFIC_NUMBERS_PATTERN = r"\b(一|二|三|四|五|六|七|八|九|十|百|千|万|亿)[一二三四五六七八九十千百千万亿个次天月年]\b"
...
def detect_ai_words(content: str) -> Dict:
    ...
    ai_probability = min(word_density * 800, 30)  # 最高贡献30%
...
def detect_ai_patterns(content: str) -> Dict:
    ...
    ai_probability = min(pattern_density * 100, 20)  # 最高贡献20%
...
def detect_sentence_complexity(content: str) -> Dict:
    ...
    contribution = 3 if uniform else 0
...
def detect_punctuation_usage(content: str) -> Dict:
    """检测标点符号使用（AI特征：规范）"""
    # 统计各种标点
...
def detect_colloquial(content: str) -> Dict:
    ...
    human_bonus = min(colloquial_density * 2000, 10)  # 最高减10%
...
def detect_metaphor(content: str) -> Dict:
    ...
    human_bonus = min(metaphor_density * 2000, 8)  # 最高减8%
...
def detect_idioms(content: str) -> Dict:
    ...
    human_bonus = min(idiom_density * 500, 8)  # 最高减8%
...
def detect_emotional(content: str) -> Dict:
    ...
    human_bonus = min(emotional_density * 1500, 10)  # 最高减10%
...
def detect_time_expressions(content: str) -> Dict:
    ...
    human_bonus = min(time_density * 1000, 7)  # 最高减7%
...
def detect_specific_numbers(content: str) -> Dict:
    """检测具体数字（人类特征）"""
    matches = re.findall(SPECIFIC_NUMBERS_PATTERN, content)
...
def detect_citations(content: str) -> Dict:
    ...
    human_bonus = min(citation_density * 500, 5)  # 最高减5%
...
def detect_repetitive_structure(content: str) -> Dict:
    ...
    contribution = 3 if repetitive else 0
...
def detect_personal_pronouns(content: str) -> Dict:
    ...
    human_bonus = min(personal_density * 500, 8)  # 最高减8%
...
def detect_data_precision(content: str) -> Dict:
    """检测数据精确度（AI特征：数据模糊）"""
    # 检测模糊数据词
    vague_words = [
        "很多", "大量", "众多", "不少", "若干",
...
def calculate_ai_score(content: str) -> Dict:
    ...
    punctuation_result = detect_punctuation_usage(content)
    ...
    vague_result = detect_data_precision(content)
    ...
    # 基础AI概率（基于长度）
    base_probability = min(40, len(content) / 100)
    ...
    dimensions = [
        {
            "name": "AI常用词",
            "type": "ai_feature",
            "value": ai_words_result["count"],
            "contribution": ai_words_result["contribution"]
        },
        {
            "name": "AI句式",
            "type": "ai_feature",
            "value": ai_patterns_result["count"],
            "contribution": ai_patterns_result["contribution"]
        },
        {
            "name": "词汇多样性",
            "type": "ai_feature",
            "value": diversity_result["diversity"],
            "contribution": diversity_result["contribution"]
        },
        {
            "name": "句子复杂度",
            "type": "ai_feature",
            "value": complexity_result["uniform"],
            "contribution": complexity_result["contribution"]
        },
        {
            "name": "口语化",
            "type": "human_feature",
            "value": colloquial_result["count"],
            "contribution": -colloquial_result["human_bonus"]
        },
        {
            "name": "比喻修辞",
            "type": "human_feature",
            "value": metaphor_result["count"],
            "contribution": -metaphor_result["human_bonus"]
        },
        {
            "name": "俗语成语",
            "type": "human_feature",
            "value": idiom_result["count"],
            "contribution": -idiom_result["human_bonus"]
        },
        {
            "name": "情感词",
            "type": "human_feature",
            "value": emotional_result["count"],
            "contribution": -emotional_result["human_bonus"]
        },
        {
            "name": "时间表达",
            "type": "human_feature",
            "value": time_result["count"],
            "contribution": -time_result["human_bonus"]
        },
        {
            "name": "第一人称",
            "type": "human_feature",
            "value": personal_result["count"],
            "contribution": -personal_result["human_bonus"]
        },
    ]
...
if __name__ == "__main__":
    ...
    for i, text in enumerate(test_texts, 1):
        ...
        for dim in result['dimensions']:
            print(f"   {dim['name']}: {dim['value']} ({dim['type']})")
